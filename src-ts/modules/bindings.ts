    ...
    switch (cond[idx]) {
      ...
      case 3:
        if (!memory.heapi32u) {
          ...
          ops.isDefined = (val: any) => val != undefined && val != null;
          ops.libwasm_encode_optional_Handle = (ptr: number, val: any) => {
      ...
      case 6:
        if (!memory.heapi8s) {
          ...
          ops.setByte = (ptr: number, val: any) => (memory.heapi8s[ptr] = val);
      ...
      case 7:
        if (!memory.heapi8u) {
          ...
          ops.setUByte = (ptr: number, val: any) => (memory.heapi8u[ptr] = val);
    ...
    if (argsdef_arr[i].startsWith('SumType!')) {
      ...
      for (let k = 0; k < argsdef_sumtype_arr.length; k++) {
        ...
        if (argsdef_sumtype_arr[k].startsWith('Handle')) {
          ...
          if (!next_val_is_null) args_arr.push(libwasm.objects[args[j][k]]);
        } else {
          //console.log("Pushing: ", args[j][k], " j: ", j, " k: ", k);
...
export let jsExports = {
  env: {
    ...
    JSON_parse_string: (valLen: number, valOffset: number) => {
      return libwasm.addObject(JSON.parse(decoder.string(valLen, valOffset)));
    },

    createCustomElement: (valLen: number, valOffset: number) => {
      return libwasm.addObject(
        document.createElement(decoder.string(valLen, valOffset))
      );
    },
    ...
    WindowOrWorkerGlobalScope_setInterval__Handle_int_Handle: (
      ...
      return window.setInterval(
        (param: any) => {
          ...
          libwasm.instance.exports.__indirect_function_table.get(CbPtr)(
            512,
            CbCtx,
            0
          );
    ...
    promise_then_6uhandlehandle: (handle: number, ctx: number, ptr: number) => {
      setupMemory([libwasm.MemoryIdentifiers.i32u]);
      let ret = libwasm.addObject(
    ...
    EventTarget_addEventListener_0: (
      ...
      libwasm.objects[ctx].addEventListener(
        ...
        (event: any) => {
          ...
          libwasm.instance.exports.__indirect_function_table.get(callbackPtr)(
            callbackCtx,
            0
          );
        }
    ...
    ldexec: (
      ...
      try {
          ...
          if (commands[idx]['local'] !== undefined) {
            ...
            if (value[0] == '=') {
              if (value[1] == '(') {
                ...
                if (value[1] == 't' && value == '=true') window[key] = true;
            ...
            for (idx2 in commands[idx]['params']) {
              ...
              if (value[0] == '=') {
                if (value[1] == '(') {
                  ...
                  if (value[1] == 't' && value == '=true') params.push(true);
                  else if (value[1] == 'f' && value == '=false')
                    params.push(false);
                  else if (value[1] >= '0' && value[1] <= '9')
        ...
        if (errorPtr > 0) {
          ...
          libwasm.instance.exports.__indirect_function_table.get(errorPtr)(
            errorCtx,
            hndl
          );
        } else {
    ...
    ldexec_long__long: (
      ...
      return BigInt(
        jsExports.env.ldexec(num, cLen, cOff, cCtx, cPtr, errCtx, errPtr, false)
      );
    },
    ldexec_long__double: (
      num: number,
