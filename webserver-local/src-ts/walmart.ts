  ...
  try {
    ...
    if (!store_selected) {
      ...
      const products_arr = _.map(
        ...
        (item: Cheerio<Element>) => {
          return {
            ...
            price_divs: _.map(
              cheerio(item)?.find('div[data-automation-id="product-price"]')[0]
                ?.children,
              (item2: Cheerio<Element>) => cheerio(item2)?.text()
            ),
          }
      ...
      const products = _.filter(
        products_arr.map((product_obj) => {
          const prod_spans = product_obj.spans
          const ret = {
            name: prod_spans[0],
            price: prod_spans
              .find((val) => val.startsWith('current price '))
              ?.replace('current price ', ''),
            price_by_weight: product_obj.price_divs.find((item) =>
              item.includes('/')
            ),
            price_before: prod_spans
              .find((val) => val.startsWith('Was '))
              ?.replace('Was ', ''),
